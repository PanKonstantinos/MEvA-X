...
def mertics_calculator(y_true, y_pred_proba, avg='weighted'):
	...
	scores = {}
	scores['Balanced_accuracy'] = balanced_accuracy_score(y_true, y_pred) # Accuracy = 1.0
...
def parsing_data_and_labels(dataset_filename=None, delimiter_dataset=None,\
	...
	if labels_filename and delimiter_labels:
		return_labels = True
		labels = pd.read_csv(labels_filename, sep = delimiter_labels, header = 0, index_col = 0, na_values = [-1000,-999,999,' ','_','-',''])
		#if as_pandas==False:
...
def mutation(selected_individuals, population, min_values, max_values, mutation_probability, generations, current_gen, mu=0, s=0.1):
	...
	for indiv in range(1, population):
		...
		if added_features_flag:
			...
			num_of_added_features = min(num_of_added_features,np.arange(selected_individuals[indiv].shape[0]-parameters)[off_genes].shape[0])
...
# 		)
		...
		for i in range(1,population-1,2):
			...
			if random_number[i]<=two_points_crossover_probability:
				...
				temp_cross_over = selected_individuals[i+1, parameters+cross_point1:parameters+cross_point2]
				selected_individuals[i+1, parameters+cross_point1:parameters+cross_point2] = selected_individuals[i, parameters+cross_point1:parameters+cross_point2]
				selected_individuals[i, parameters+cross_point1:parameters+cross_point2] = temp_cross_over
...
if __name__ == "__main__":
	opera_label = int(input('Select the category [1-4]: '))
	prog_time = time.time()
	#####
	...
	ans = str(input('Use the default weights for the metrics? (Y/N): '))
	...
	output_folder = os.path.join(dname,f'XGB_Opera{opera_label}_Results\\'+f'P{population}_G{generations}_K{num_of_folds}'+'_'+str(time.time())+'\\')
