...
class Individual():
    ...
    def __init__(self, individual, process_i, dataset, labels, num_of_folds, filter_mask, multiclass=True, random_state=None, last_eval=False, feature_names=None, verbose=True):
    ...
    def evaluate(self):
        ...
        if self.process_i==0 and not self.verbose:
...
def majority_voting(individuals, dataset, labels, filter_mask, index, num_of_folds, N_trees, multiclass, output_folder, par_type, feature_names, verbose):
    ...
    if verbose:
        ...
        print('Array of 1st Pareto feature names:\n',feature_names[pareto1_intersected_features])
...
def weighted_geometric_mean(y_true,y_pred,unique_labels=None, verbose=True):
    """
    MCM[:,0,0] => TN
    MCM[:,1,0] => FN
    MCM[:,1,1] => TP
    ...
    if MCM.shape[0]==2:
        ...
        for i in range(MCM.shape[0]):
            ...
            GM += geo_mean*support
            if verbose: print(f'Support for class_{i} = {support} and GM = {geo_mean}')
        GM = GM/support_sum
    return GM
...
def evaluate_individuals(dataset, labels, individuals, goal_significances, num_of_folds, classification_problems,
						output_folder, JMI_genes, Wilcoxon_genes, mRMR_genes, SelKBest_genes, multiclass=True, verbose=True):
    ...
    for i, individual in enumerate(individuals):
        res,mean_std, roc_auc_indiv = evaluate_individual(Individual(individual, i, dataset, labels, num_of_folds, filter_mask[i], multiclass, random_state, verbose))
        results.append(res)
    ...
    if verbose: print(f'N_trees = {N_trees}')
...
def mifs_calc(data, labels, method = 'JMI', n_features = 100, k_vals = 4):
    ...
    try:
        FS_methods = np.empty((len(k_vals)*len(method),n_features)) # if k_vals is a list
    except:
        FS_methods = np.empty((k_vals*len(method),n_features)) # if k_vals is an integer
    i = 0
    ...
    for meth in method:
        for k in k_vals:
            ...
            i+=1
    if verbose: print(f'The shape is:{FS_methods.shape} and Transposed is {FS_methods.T.shape}')
    return FS_methods
...
def filter_function(individuals, JMI_genes, Wilcoxon_genes, mRMR_genes, SelKBest_genes, verbose):
    '''
            ...
            if int(individuals[indiv,1]) < 2:
                ...
                if int(individuals[indiv,0]) == 1: #JMI
                    if JMI_genes is None:
                        ...
                        if verbose: print('JMI FS has selected no features. No filtering will apply on the features!')
                        continue
                    else:
                    ...
                    if Wilcoxon_genes is None:
                        filter_mask[indiv,parameters:] = filter_mask[indiv,parameters:].round()
                        if verbose: print('Wilcoxon rank sums FS has selected no features. No filtering will apply on the features!')
                        continue
                    else:
                    ...
                    if mRMR_genes is None:
                        filter_mask[indiv,parameters:] = filter_mask[indiv,parameters:].round()
                        if verbose: print('mRMR FS has selected no features. No filtering will apply on the features!')
                        continue
                    else:
                    ...
                    if SelKBest_genes is None:
                        filter_mask[indiv,parameters:] = filter_mask[indiv,parameters:].round()
                        if verbose: print('Select K Best FS has selected no features. No filtering will apply on the features!')
                        continue
                    else:
...
def similarity_function(fronts, evaluation_values, individuals, sigma_share, max_values, min_values):
    ...
    for front in sorted(np.unique(fronts)):
        ...
        for j in range(ind.shape[0]): # 0,1,2,3,4 (e.g. len(ind) = 5)
            ...
            ms[i] += m
        ms[i] /= ind.shape[0]
        if verbose: print(f'Mean m for frontier {front} ({ind.shape[0]}) = {ms[i]}')
        i+=1
...
def apply_evolutionary_process(generations, population, max_values, min_values, two_points_crossover_probability,
								...
								JMI_genes, Wilcoxon_genes, mRMR_genes, SelKBest_genes, eval_names, multiclass=True, verbose=False, to_plot=False):
    ...
    for rep in range(generations):
        ...
        evaluation_values, mean_std_list, roc_auc_list, _ = evaluate_individuals(dataset, labels, individuals,
                                                                           ...
                                                                           output_folder, JMI_genes, Wilcoxon_genes, mRMR_genes, SelKBest_genes, multiclass, verbose)
        ...
        if to_plot:
            ...
            ax.set(title=f"ROC/AUC MEvA-X Population:{population} Generation:{rep}")
        ...
        if math.fabs(evaluation_values[-1,0]-average_performance)<10E-4*average_performance: #Check if the 'Best' individual is almost equal to average
            #premature_termination=1
            if verbose: print('Premature termination!')
            break
        ...
        if best_indiv_pos==0:
            if verbose: print('Individual_0 is the best solution again!')
        else:
...
def biomarker_discovery_modeller(dataset, feature_names, sample_names, labels, min_values, max_values, population,
								...
								eval_names=None, missing_values_flag=True, normalize_flag=False, FS_calc=False, k_vals=[6,7], verbose=True, to_plot=False):
    '''
    ...
    -----------
        ...
        FS_calc [bool]: Selects if the Feature selection methods should be calculated or not
        k_vals [list of ints]: the k in the k-NN for the feature selection techniques
        verbose [bool]: Printing option selection. True prints less text
    Return:
    -----------
    '''
    ...
    multiclass = True
    # If there is a two class problem, don't use Manhattan_dist
    if unique_labels.shape[0]==2:
        multiclass = False
        to_del = np.argwhere(np.chararray.startswith(eval_names,"Manhattan")).squeeze()
        if to_del:
            eval_names = np.delete(eval_names,to_del)
            ## If the length of the eval_names -1 (for the Overall score) is shorter than goal_significances we need to delete the Manhattan's value
            if (eval_names.shape[0] - 1) < goal_significances.shape[0]:
                goal_significances = np.delete(goal_significances,to_del)
    ...
    if feature_names.shape[0]>200: n_features = 100
    else: n_features = feature_names.shape[0]//2
    ...
    if FS_calc:
        try:
            ...
            if not os.path.exists(os.path.join(output_folder,'Feature_selection')):
                os.path.makedirs(os.path.join(output_folder,'Feature_selection'))
            JMI_genes.to_csv(f"{os.path.join(output_folder,'Feature_selection','JMI.tsv')}", sep='\t')
        except:
            if verbose: print('JMI Feature selection failed. No filtering will apply on the features.')
            JMI_genes = None
        ...
        try:
            ...
            if not os.path.exists(os.path.join(output_folder,'Feature_selection')):
                os.path.makedirs(os.path.join(output_folder,'Feature_selection'))
            mRMR_genes.to_csv(f"{os.path.join(output_folder,'Feature_selection','mRMR.tsv')}", sep='\t')
        except:
            if verbose: print('mRMR Feature selection failed. No filtering will apply on the features.')
            mRMR_genes = None
        ...
        try:
            ...
            if not os.path.exists(os.path.join(output_folder,'Feature_selection')):
                os.path.makedirs(os.path.join(output_folder,'Feature_selection'))
            Wilcoxon_genes.to_csv(f"{os.path.join(output_folder,'Feature_selection','Wilcoxon_ranksums.tsv')}", sep='\t')
        except:
            if verbose: print('Wilcoxon rank sums Feature selection failed. No filtering will apply on the features.')
            Wilcoxon_genes = None

        try:
            k_bst=n_features #100
            ...
            if not os.path.exists(os.path.join(output_folder,'Feature_selection')):
                os.path.makedirs(os.path.join(output_folder,'Feature_selection'))
            SelKBest_genes.to_csv(f"{os.path.join(output_folder,'Feature_selection','Select_K_Best.tsv')}", sep='\t')
        except:
            if verbose: print('Select K Best (SKB) Feature selection failed. No filtering will apply on the features.')
            SelKBest_genes = None
    ## Read the precalculated selected features
    else:
        # JMI
        try:
            JMI_genes = pd.read_csv(os.path.join(FS_dir,'JMI.tsv'), sep='\t', header = None).to_numpy().squeeze().astype(np.int32,copy=False)
        except:
            if verbose: print('No such a file found for JMI. No filtering will apply on the features')
            JMI_genes = None

        # mRMR
        try:
            mRMR_genes = pd.read_csv(os.path.join(FS_dir, 'mRMR.tsv'), sep='\t', header = None).to_numpy().squeeze().astype(np.int32,copy=False)
        except:
            if verbose: print('No such a file found for mRMR. No filtering will apply on the features')
            mRMR_genes = None

        # Wilcoxon
        try:
            Wilcoxon_genes = pd.read_csv(os.path.join(FS_dir, 'Wilcoxon_ranksums.tsv'), sep='\t', header = None).to_numpy().squeeze().astype(np.int32,copy=False)
        except:
            if verbose: print('No such a file found for Wilcoxon rank sums. No filtering will apply on the features')
            Wilcoxon_genes = None

        # SKB
        try:
            SelKBest_genes = pd.read_csv(os.path.join(FS_dir, 'Select_K_Best.tsv'), sep='\t', header = None).to_numpy().squeeze().astype(np.int32,copy=False)
        except:
            if verbose: print('No such a file found for Select k best. No filtering will apply on the features')
            SelKBest_genes = None
    ...
    majority_voting(individuals, dataset, labels, filter_mask, pareto1_indx, num_of_folds, N_trees, multiclass, output_folder, par_type='full_pareto1', feature_names=feature_names, verbose=verbose)
    print("Done!")

    print('Highest Pareto values majority voting')
    majority_voting(individuals, dataset, labels, filter_mask, pareto1_indx[pareto1_maximums], num_of_folds, N_trees, multiclass, output_folder, par_type='top_pareto1', feature_names=feature_names, verbose=verbose)
    print("Done!")
    ...
    print(f'The Results are saved in the directory: {os.path.abspath(output_folder)}')
...
def get_parser():
    ...
    MEvAX_args = argparse.ArgumentParser(prog='MEvA-X', description='A hybrid algorithm for feature selection, hyper-parameter optimization and model training.\nMEvA-X uses a combination of a Nitched Pareto Evolutionary algorithm and the XGBoost Classifier to achieve the above-mentioned objectives.\nThe end result is a cluster of equally acceptale trained models solving the given problem.', epilog='This algorithm is the result of the work of K. Panagiotopoulos, A. Korfiati, K. Theofilatos, M.A. Deriu, and S. Mavroudi')

    MEvAX_args.add_argument("--dataset_path", "-A", type=file_path,  default="data.csv", required=True, dest='dataset_filename', help="[str]: The path to the file containing the data. Format expected: FeaturesXSamples") # "*" -> 0 or more values expected => creates a list
    MEvAX_args.add_argument("--labels_path", "-B", type=file_path,  default="labels.csv", required=True, dest='labels_filename', help="[str]: The path to the file containing the labels of the data. Sample names should not be used")
    MEvAX_args.add_argument("--FS_dir", type=dir_path,  default=None, help="[str]: The path to the directory containing precalculated features from the Feature Selection techniques (mRMR, JMI, Wilcoxon, and SelectKBest)")
    MEvAX_args.add_argument("--output_dir", type=str,  default=os.path.join(os.getcwd(), "Results"), dest='output_folder')
    MEvAX_args.add_argument("--K", '-k', type=int, default=10, dest='num_of_folds', help="[int]: The number of folds to be used in the K-fold cross validation. Default = 10")
    MEvAX_args.add_argument("--P", '-p', type=int, default=50, dest='population', help="[int]: The number of individual solutions. Default = 50")
    MEvAX_args.add_argument("--G", '-g', type=int, default=100, dest='generations', help="[int]: The number of maximum generations for the Evolutionry Algorithm. Default = 100")
    MEvAX_args.add_argument("--crossover_perc", type=float, default=0.9, dest='two_points_crossover_probability', help="[float]: Probability of a two-point crossover for the creation of a new offsping. Default = 0.9")
    MEvAX_args.add_argument("--arithmetic_perc", type=float, default=0.0, dest='arithmetic_crossover_probability', help="[float]: Probability of an arithmetic crossover for the creation of a new offsping. Default = 0.0")
    MEvAX_args.add_argument("--mutation_perc", type=float, default=0.05, dest='mutation_probability', help="[float]: The probability of point mutations to occure in the genome of an offspring. Default = 0.05")
    MEvAX_args.add_argument("--goal_sig_path", "-gsp", type=file_path, default=None, dest='goal_significances_path', help="[str]: The path to the file containing the weights of the evaluation metrics")
    MEvAX_args.add_argument("--goal_sig_list", "-gsl", nargs="*", type=float, default=[0.8,0.8,0.8,2,1,1,1,1,2,0.5,2], dest='goal_significances_list', help="[array-like of floats]: The array of the weights for the evaluation metrics. Default = [0.8, 0.8, 0.8, 2, 1, 1, 1, 1, 2, 0.5, 2]")
    MEvAX_args.add_argument("--impute", type=lambda x:bool(strtobool(x)), default=True, dest='missing_values_flag', help="[bool]: Select if the missing values should be imputed with a KNN imputer. Default = True")
    MEvAX_args.add_argument("--normalize", type=lambda x:bool(strtobool(x)), default=True, dest='normalize_flag', help="[bool]: Select if the data should get normalized. If \'True\' it normalizes the values between the intervals [0, 1] or [-1, 1] if there are negative values. Default = True")
    MEvAX_args.add_argument("--verbose", "-V", type=lambda x:bool(strtobool(x)), default=False, dest='verbose', help="[bool]: The option of printing more information. If \'False\' only information of the program essentials will be printed. Default = False")
    MEvAX_args.add_argument("--plot", "-plt", type=lambda x:bool(strtobool(x)), default=False, dest='to_plot', help="[bool]: The option of plotting and saving the AUC of the trainngs. If \'True\' it produces AUC plots for every itteration but also slows down the algorithm. Default = False")
...
def file_path(path):
    if os.path.isfile(path):
        ...
        raise argparse.ArgumentTypeError(f"readable_dir:{path} is not a valid path for a file")
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(output_folder):
        ...
        print(f"{output_folder} derectory created at {os.path.abspath(output_folder)}")
    ...
    if args.goal_significances_path:
        goal_significances_filename = args.goal_significances_path
        goal_significances = pd.read_csv(os.path.abspath(goal_significances_filename), header=None).values.squeeze()
        if goal_significances.shape[0]>1:
            try:
                goal_significances = np.asarray(goal_significances[-1,:], dtype=float)
            except:
                goal_significances = np.asarray(goal_significances, dtype=float)
    else:
        goal_significances = np.asarray(args.goal_significances_list, dtype=float).squeeze()

    #Select if normalization and imputation will take place
    missing_values_flag = args.missing_values_flag
    ...
    if output_folder and verbose:
        print(f"Output folder: {output_folder}")

    output_folder = os.path.join(output_folder, f"Models_P{population}_G{generations}_K{num_of_folds}_{str(time.time_ns())[:-9]}\\")
    if verbose:
        print(output_folder)
    ...
    biomarker_discovery_modeller(dataset, feature_names, sample_names, labels, min_values, max_values, population,
                                ...
                                eval_names, missing_values_flag, normalize_flag, FS_calc, k_vals, verbose, to_plot)
